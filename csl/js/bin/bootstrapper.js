...
function handleDiscoverRequest(socket, outBuffer, address, port) {
    ...
    if (net[networkInterface] && net[networkInterface].length > 0
        ...
        for (var key in net) {
            if (!key.match(/^lo\d?$/) && (key.indexOf('Loopback') < 0) && (net[key].length > 0)
                && net[key][0].mac && (net[key][0].mac !== info.MAC)) {
                info.MAC = net[key][0].mac;
                break;
            }
        }
    }
    ...
    var offset = 1;
    for (var key in info) {
        offset += encodeLengthValue(key, outBuffer, offset);
        offset += encodeLengthValue(info[key], outBuffer, offset);
    }
